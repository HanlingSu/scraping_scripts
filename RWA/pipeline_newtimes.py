...
def collect_urls_sitemap(num):  
    ...
    '''
    urls=[]
    url = f"https://www.newtimes.co.rw/sitemap.xml?page={num}"
...
def collect_urls_year_month_day(yy, mm, days): #(keyword[0],keyword[1])
    ...
    for dd in days:
        ...
        try:
            ...
            for i in soup.find_all('a', {'class':"eltdf-pt-link", 'itemprop':'url'}):
                link = i['href']
                urls.append(link)
...
def collect_urls_keyword(keyword, page):
    ...
    try:
        #### change url pattern
        page_url = f'https://www.newtimes.co.rw/morearticles/news/{keyword}?pgno={page}'
        page_req = requests.get(page_url,headers=headers)
        page_soup = BeautifulSoup(page_req.content, features="lxml")
        for i in page_soup.find_all('div', {'class':"article-title"}):
            b = i.find('a')
            link = b['href']
            urls.append(link)
...
def pipeline(sitemap,
    ...
    if sitemap:
        ...
        begin_num = 38
        end_num = 39
        ll = list(range(begin_num, end_num))[::-1]
        for num in ll:
    ...
    if keyword:
        ...
        keywords = [('africa',16), ('rwanda',20), ('business',10), ('africa',8), ('international',8), ('technology',5), ('agriculture',4), ('coronavirus',1), 
        ('health',3), ('energy',3), ('infrastruture',2), ('crime',3), ('finance',2), ('law',3), ('housing',2),
        ('aviation',2), ('featured',6), ('tourism',5), ('culture',5), ('economy',7), ('education',7), ('security',6) ]
        # keywords =[]
