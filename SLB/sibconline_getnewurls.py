...
for i in range(11, 12): # smaller numbered-sitemaps host the more recent stuff.
...
for url in final_result:
    if url:
        ...
        try:
            ...
            try:
                category = soup.find('span', {'class' : 'fancy-categories'}).text
            except:
                category = "News"
            print(category)
            blacklist = ['Entertainment', 'Sports', 'Religion', 'Design & Photography', 'Letters to the Editor']
            if any(substr in category for substr in blacklist):
                article['date_publish'] = None
                article['title'] = "From uninterested category"
                article['maintext'] = None
            try:
                date = soup.find('time', {'class' :'entry-date updated'})['datetime']
            ...
            try:
                ...
                if any(substr in category for substr in ['Editorial', 'Viewpoint']):
                    colname = f'opinion-articles-{year}-{month}'
                    article['primary_location'] = 'SLB'
                else:
