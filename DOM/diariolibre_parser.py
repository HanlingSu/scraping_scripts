...
class UpdateDB:
    ...
    def __init__(self, col_to_scrape = None,
        ...
        self.dates = pd.date_range(start=datetime(start_year,3,1), end=datetime(start_year,4,1), freq='m')
...
#################
    ...
    def kompas_story(self, soup):
        ...
        # Get Date  
        try:
            date  = soup.find('meta', {'name' : 'ArticlePublicationDate'})['content']
            hold_dict['date_publish'] = dateparser.parse(date).replace(tzinfo = None)
        except:
            date = soup.find('time', {'id' : 'detail-datetime'})['datetime']
