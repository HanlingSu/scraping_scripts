...
source = 'https://www.environnement.gov.bf'
direct_URLs = set()

for p in range(1, 38):
    link = 'https://www.environnement.gov.bf/informations/actualites?tx_news_pi1%5B%40widget_0%5D%5BcurrentPage%5D=' + str(p) +'&cHash=014bbfa6f33c7e089801d3eac16b4edd'
    print("Getting urls from: ", link)
    
    response = scraper.get(link)
    soup = BeautifulSoup(response.text, 'html.parser')

    for i in soup.find_all('div', {'class': 'actualite-item-header'}):
        try:
            a_tag = i.find('a')
            if a_tag and a_tag.has_attr('href'):
                relative_url = a_tag['href']
                full_url = urljoin(source, relative_url)
