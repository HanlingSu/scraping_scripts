...
for p in range(1, 99):
    url = 'https://www.mgtc.gov.my/category/in-the-news/page/' + str(p) + '/'
...
for url in final_result:
    if url:
        ...
        try:
            ...
            # Else, from the published date 
            else:
                try:
                    date = soup.find('span', {'class':'elementor-post-date'}).text.strip()
                    article['date_publish'] = dateparser.parse(date)
                except:
                    print("No date found in page or URL")
