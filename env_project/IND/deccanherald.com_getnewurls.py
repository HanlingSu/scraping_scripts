...
for p in range(2, 75):
    link = 'https://www.deccanherald.com/climate-environment/' + str(p) 
    print("Getting urls from: ", link)

    response = requests.get(link, verify=False)
    soup = BeautifulSoup(response.content)

    container = soup.find('div', {'class': 'container _30XCi'})
    for i in container.find_all('div', {'class':'DaSgX'}):
        a_tag = i.find('a')
...
for url in final_result:
    if url:
        ...
        try:
            ...
            #Extract date: 
            try: 
                date = soup.find('div', {'class': 'ITNYL'}).text
