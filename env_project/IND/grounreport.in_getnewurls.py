...
direct_URLs = set()
categories = ['environment', 'extreme-weather', 'water', 'groundreport']
pages = (177, 61, 12, 29)
#177

for c, max_p in zip(categories, pages):
    for p in range(2, max_p + 1): 
#for p in range(140, 178):
        link = 'https://groundreport.in/' + str(c) + '?page=' + str(p)
    # link = 'https://groundreport.in/environment?page=' + str(p)
        ...
        try:
            ...
            for i in soup.find_all('div', {'class': 'category-article'}):
                a_tag = i.find('a')
...
for url in final_result:
    if url:
        ...
        try:
            ...
            # Extract date 
            try: 
                date = soup.find('time', {'class': 'updated_date'}).text
