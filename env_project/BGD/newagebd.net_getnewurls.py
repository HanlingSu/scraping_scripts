...
for p in range(1, 31):
    link = 'https://www.newagebd.net/articlelist/45/environment-climate-change?page=' + str(p) 
    print("Getting urls from: ",link)
    response = requests.get(link, verify=False)

    soup = BeautifulSoup(response.content)
    for i in soup.find_all('h2', {'class': 'card-title h5 h4-sm h3-md h2-lg'}):
...
for url in final_result:
    if url:
        ...
        try:
            ...
            # Extract date 
            try:
                date = soup.find('time', {'class': 'ms-0 ms-sm-2 ms-md-3'}).text.strip()
