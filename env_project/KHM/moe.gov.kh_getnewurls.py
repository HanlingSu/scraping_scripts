...
for p in range(1, 159):
    url = 'https://www.moe.gov.kh/index/category/%E1%9E%96%E1%9F%90%E1%9E%8F%E1%9F%8C%E1%9E%98%E1%9E%B6%E1%9E%93%E1%9E%90%E1%9F%92%E1%9E%98%E1%9E%B8%E1%9F%97/page/' + str(p) 
    print("Getting urls from: ", url)

    soup = BeautifulSoup(scraper.get(url).text)

    for i in soup.find_all('div', {'class':'block-body'}):
...
for url in final_result:
    if url:
        ...
        try:
            ...
            # Extract text 
            try:
                maintext = soup.find('div', {'class': 'main-content col-sm-12 col-md-8'}).text
                article['maintext'] = maintext.stip()
            except: 
                maintext = ""
                for i in soup.find('div', {'class': 'main-content col-sm-12 col-md-8'}).find_all('p'):
                    if i:
                        maintext += i.text
