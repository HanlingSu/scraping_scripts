...
for p in range(2, 223):
    link = 'https://www.times.co.zm/?s=environment&paged=' + str(p) 
    print("Getting urls from: ", link)

    response = requests.get(link, verify=False)
    soup = BeautifulSoup(response.content)
    
    for i in soup.find_all('div', {'class': 'mag-super-title'}):
...
for url in final_result:
    if url:
        ...
        try:
            ...
            try:
                date = soup.find('span', {'class': 'single-date'}).text
                date_clean = ' '.join(date.split()[:5])
                date_clean = ' '.join(date_clean.split()[2:])
                print(date_clean)
