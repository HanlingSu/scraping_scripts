...
source = 'levert.ma'
direct_URLs = set()
categories = ['le-vert-afrique', 'le-vert-monde', 'opinions-vert']
##'developpement-durable', 'transition-energetique', 'agriculture-durable', 'eaux-et-forets', 'tourisme-durable', 'vie-pratique',
pages = (42, 106, 16)
# 152, 69, 45, 49, 23, 15, 
for c, max_p in zip(categories, pages):
    for p in range(1, max_p + 1):
# for p in range(2, 3):
        link = 'https://levert.ma/category/' + str(c) + '/page/' + str(p) + '/'
        print("Getting urls from: ", link)

        response = requests.get(link, verify=False)
        soup = BeautifulSoup(response.content)

        for i in soup.find_all('h3', {'class': 'entry-title mh-loop-title'}):
