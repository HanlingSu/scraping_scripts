...
for p in range(300,500): 
    # url = 'https://neweralive.na/category/agriculture/page/' + str(p) + '/'
    url  = 'https://neweralive.na/page/' + str(p) + '/?s=environment'
    print("Getting urls from: ", url)

    soup = BeautifulSoup(scraper.get(url).text)
    for i in soup.find_all('h3', {'class':'cmsmasters_archive_item_title entry-title'}):
...
for url in final_result:
    if url:
        ...
        try:
            ...
            # Extract date
            try: 
                date = soup.find('span', {'class': 'cmsmasters_post_date'})
                date_text = date.find('abbr', {'class': 'published'}).text.strip()
                article['date_published'] = dateparser.parse(date_text)
            except:
