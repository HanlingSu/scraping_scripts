...
source = 'mediu.gov.md'
direct_URLs = set()
base_url = 'https://www.mediu.gov.md'


for p in range(1, 20): 
    link = 'https://www.mediu.gov.md/ro/press-release-types/nout%C4%83%C8%9Bi?page=' + str(p) 
    ...
    for i in soup.find_all('li', {'class':'node-readmore first last'}):
        a_tag = i.find('a')
...
for url in final_result:
    if url:
        ...
        try:
            ...
            try:
                title = soup.find('div', {'class' : 'panel-pane pane-node-content pressreleases'}).find('h2').text.strip()
                article['title'] = title
            except:
                pass
