...
direct_URLs = set()
categories = ['environment', 'energy', 'mining', 'oil-gas']
pages = (7, 7, 2, 2)

for c, max_p in zip(categories, pages):
    for p in range(1, max_p): 
        url = 'https://newbusinessethiopia.com/topics/' + str(c) + '/page/' + str(p) + '/'
        print("Getting urls from: ", url)

        soup = BeautifulSoup(scraper.get(url).text)

        for i in soup.find_all('article', {'class': 'small'}):
                title = i.find('h4', {'class': 'title'})
                if title and title.find('a'):
                    direct_URLs.add(title.find('a')['href'])
...
for url in final_result:
    if url:
        ...
        try:
            ...
            # Extract date 
            try:
                date = soup.find('span', {'class': 'bs-blog-date'}).text.strip()
