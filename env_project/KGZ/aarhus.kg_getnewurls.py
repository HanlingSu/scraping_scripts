...
for l, max_p in zip(languages, pages):
    for p in range(1, max_p + 1):
        ...
        link = 'https://aarhus.kg/' + str(l) + '/page/' + str(p) 
        print("Getting urls from: ",link)
        response = requests.get(link, verify=False)

        soup = BeautifulSoup(response.content)
        for i in soup.find_all('h2', {'class' : 'post-title entry-title'}):
...
for url in final_result:
    if url:
        ...
        try:
            ...
            # Extract date 
            try:
                date = soup.find('time', {'class': 'published'}).text.strip()
                print(date)
            ...
            try:
                maintext = ''
                for i in soup.find('div', {'class': 'entry-inner'}).find_all('p'):
                    maintext += i.get_text(strip=True) + '\n'
