...
source = 'novosti.rs'
direct_URLs = set()
base_url = 'https://www.novosti.rs'
categories = ['odrzivi-razvoj', 'ekonomija/poljoprivreda']
pages = (32, 5)

for c, max_p in zip(categories, pages):
# for p in range(2, 3): 
    for p in range(1, max_p + 1):
        url = 'https://www.novosti.rs/' + str(c) + '/page/' + str(p)
        print("Getting urls from: ", url)

        response = requests.get(url, verify=False)
        soup = BeautifulSoup(response.content)

        for i in soup.find_all('h3', {'class': 'news-item-title mb-0'}):
                a_tag = i.find('a')
