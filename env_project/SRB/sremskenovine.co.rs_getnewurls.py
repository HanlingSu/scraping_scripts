...
source = 'sremskenovine.co.rs'
direct_URLs = set()


for p in range(1, 73): 
    url = 'https://www.sremskenovine.co.rs/category/ekonomija/poljoprivreda/page/' + str(p)
...
for url in final_result:
    if url:
        ...
        try:
            ...
            # Extract maintext 
            try: 
                maintext = soup.find('div', {'class': 'single-post-content'}).text 
                article['maintext'] = maintext.strip()
            except:
                maintext = ""
                for i in soup.find('div', {'class': 'single-post-content'}).find_all('p'):
                    maintext += i.text
                    article['maintext'] = maintext.strip()
