...
source = 'glaspodrinja.rs'
direct_URLs = set()


for p in range(601, 812): 
    url = 'https://www.glaspodrinja.rs/vesti/rubrika/poljoprivreda/' + str(p)
    print("Getting urls from: ", url)

    response = requests.get(url, verify=False)
    soup = BeautifulSoup(response.content)

    for i in soup.find_all('h2', {'class': 'gfont'}):
...
for url in final_result:
    if url:
        ...
        try:
            ...
            # Extract date 
            try: 
                date = soup.find('span', {'class': 'desk'}).text.strip()
