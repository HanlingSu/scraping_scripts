...
direct_URLs = set()
base_url = 'https://www.glas-javnosti.rs/'


for p in range(2, 799): 
    url = 'https://www.glas-javnosti.rs/okruzenje?page=' + str(p)
    print("Getting urls from: ", url)

    response = requests.get(url, verify=False)
    soup = BeautifulSoup(response.content)

    for i in soup.find_all('div', {'class': 'news-text'}):
        a_tag = i.find('a')
...
for url in final_result:
    if url:
        ...
        try:
            ...
            # Extract maintext 
            try: 
                maintext = soup.find('div', {'class': 'newsok'}).text 
                article['maintext'] = maintext.strip()
            except:
                maintext = ""
                for i in soup.find('div', {'class': 'newsok'}).find_all('p'):
                    maintext += i.text
                    article['maintext'] = maintext.strip()
