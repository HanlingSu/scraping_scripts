...
categories= ['policy', 'climate-justice', 'energy', 'land', 'transport', 'comment']
page_start = [1, 1, 1, 1, 1, 1]
page_end = [233, 15, 104, 61, 30, 60]
...
for c, ps, pe in zip(categories, page_start, page_end):
    for i in range(ps, pe+1):
        direct_URLs = []
        link = 'https://www.climatechangenews.com/category/' + c + '/page/' + str(i)
        print(link)

        hdr = {'User-Agent': 'Mozilla/5.0'}
        req = requests.get(link, headers = hdr)
        soup = BeautifulSoup(req.content)
        items = soup.find('div', {'class' : 'content border-between'}).find_all('h2')
        for item in items:
