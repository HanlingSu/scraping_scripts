...
for p in range(1, 201): 
    url = 'https://www.gob.pe/institucion/minam/noticias?sheet=' + str(p)
    print("Getting urls from: ", url)

    response = requests.get(url, verify=False)
    soup = BeautifulSoup(response.content)

    for header in soup.find_all('li', {'class': 'scrollable__item mb-8 col-md-4 col-lg-3'}):
        title_tag = header.find('h3', {'class':'text-base leading-snug flex-1 mt-4 sm:mt-0'})
        date_tag = header.find('time', {'class': 'block text-xs text-gray-500 z-10 mt-2'})
        link_tag = title_tag.find('a') if title_tag else None

        if link_tag and 'href' in link_tag.attrs and date_tag and 'datetime' in date_tag.attrs:
            href = link_tag['href']
            url = base_url + href 
