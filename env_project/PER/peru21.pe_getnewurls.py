...
# 34
for p in range(1, 35): 
    url = 'https://peru21.pe/noticias/medio-ambiente/' + str(p) + '/'
    print("Getting urls from: ", url)

    soup = BeautifulSoup(scraper.get(url).text)

    container = soup.find_all('div', {'class': 'titulo-teaser-2col'})
...
for url in final_result:
    if url:
        ...
        try:
            ...
            try:
                maintext = soup.find('div', {'class': 'articulo-contenido-full content'}).text
                article['maintext'] = maintext.strip()
            except:
                maintext = ""
                for i in soup.find('div', {'class': 'articulo-contenido-full content'}).find_all('div', {'class': 'field__item'}):
                    if i:
                        maintext += i.text
                article['maintext'] = maintext.strip()
            ...
            """ 
            try:
                date = soup.find('time', {'class': 'datetime'}).text.strip()
                print(date)
                article['date_publish'] = dateparser.parse(date)
            except:
