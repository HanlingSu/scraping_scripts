...
categories = ['energjetika', 'bujqesi']
pages = (125, 70)

for c, max_p in zip(categories, pages):
    for p in range(1, max_p + 1):
        link = 'https://telegrafi.com/category/ekonomi/' + str(c) + '/page/' + str(p) 
        print("Getting urls from: ", link)

        response = requests.get(link, verify=False)
        soup = BeautifulSoup(response.content)

        for i in soup.find_all('a', {'class':'post__large post__large--row'}):
            href = i.get('href')
            if href:
...
for url in final_result:
    if url:
        ...
        try:
            ...
            try:
                maintext = ''
                for i in soup.find('div', {'class': 'single__article--content'}).find_all('p'):
