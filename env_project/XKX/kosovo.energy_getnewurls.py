...
categories = ['renewable-energy', 'environmental-news', 'regional-and-international-news', 'traditional-energy']
pages = (121, 71, 329, 30)

for c, max_p in zip(categories, pages):
    for p in range(1, max_p + 1):
        link = 'https://kosovo.energy/category/' + str(c) + '/page/' + str(p) 
        print("Getting urls from: ", link)
        
        # response = requests.get(link, verify=False)
        response = requests.get(link, headers=hdr, verify=False, timeout=10)
