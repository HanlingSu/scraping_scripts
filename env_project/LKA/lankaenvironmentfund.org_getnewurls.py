...
source = 'lankaenvironmentfund.org'
direct_URLs = set()

for p in range(1, 26):
    link = 'https://www.lankaenvironmentfund.org/news?dcdde12f_page=' + str(p) 
    print("Getting urls from: ", link)

    response = requests.get(link, verify=False)
    soup = BeautifulSoup(response.content)

    for i in soup.find_all('div', {'class': 'div-block-10 w-dyn-item'}):
        a_tag = i.find('a')
        if a_tag and 'href' in a_tag.attrs:
            full_url = 'https://www.lankaenvironmentfund.org' + a_tag['href']
...
for url in final_result:
    if url:
        ...
        try:
            ...
            # Extract date 
            try:
                date = soup.find('h1', {'class': 'heading-11'}).text
