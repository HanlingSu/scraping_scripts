...
source = 'disasterdisplacement.org'

direct_URLs = []

for p in range(1, 43):
    link = 'https://disasterdisplacement.org/news-event/page/' + str(p) + '/'
    print("Getting urls from: ",link)
    response = requests.get(link, verify=False)

    soup = BeautifulSoup(response.content)
    for i in soup.find_all('h2', {'class': 'wpex-card-title wpex-heading wpex-text-lg wpex-mb-5'}):
