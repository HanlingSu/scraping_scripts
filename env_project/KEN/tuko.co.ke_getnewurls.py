...
for p in range(1, 18):
    link = 'https://www.tuko.co.ke/tags/climate-change/?page=' + str(p) 
    print("Getting urls from: ", link)

    response = requests.get(link, verify=False)
    soup = BeautifulSoup(response.content)

    for i in soup.find_all('div', {'class':'c-article-card-horizontal__container'}):
...
for url in final_result:
    if url:
        ...
        try:
            ...
            try: 
                date = soup.find('div', {'class': 'c-article-info post__info'}).text
                date_clean = date.replace("Updated", "").strip() 
                date_clean = ' '.join(date_clean.split()[:6])
                print(date_clean)
