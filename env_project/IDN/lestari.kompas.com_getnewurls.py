...
for p in range(437, 438):
    url = 'https://lestari.kompas.com/category/lingkungan?page=' + str(p)
    print("Getting URLs from:", url)

    soup = BeautifulSoup(scraper.get(url).text)

    for i in soup.find_all('div', {'class':'articleItem'}):
...
for url in final_result:
    if url:
        ...
        try:
            ...
            # Extract text 
            try:
                maintext = soup.find('div', {'class': 'clearfix'}).text
                article['maintext'] = maintext.stip()
            except: 
                maintext = ""
                for i in soup.find('div', {'class': 'clearfix'}).find_all('p'):
                    if i:
                        maintext += i.text
