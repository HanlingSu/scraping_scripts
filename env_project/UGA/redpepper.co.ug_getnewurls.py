...
for p in range(1, 174):
    link = 'https://redpepper.co.ug/page/' + str(p) + '/?s=environment'
    # link = ' https://redpepper.co.ug/category/business/agriculture/page/' + str(p)
    ...
    for i in soup.find_all('h3', {'class':'article-title article-title-1'}):
...
for url in final_result:
    if url:
        ...
        try:
            ...
            try:
                date = soup.find('span', {'class' : "item-metadata posts-date"})
                # date = soup.find('span', {'class' : "date_detail"}).text.strip()
