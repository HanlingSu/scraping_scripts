...
for p in range(2, 3):
    link = 'https://www.planetaid.org/blog/page/' + str(p) 
    print("Getting urls from: ", link)

    response = requests.get(link, verify=False)
    soup = BeautifulSoup(response.content)
    
    for i in soup.find_all('article', {'class': 'blog-card blog-card--small aos-init aos-animate'}):
...
for url in final_result:
    if url:
        ...
        try:
            ...
            # Extract maintext 
            try: 
                maintext = soup.find('div', {'class': 'entry clearfix'}).text
                article['maintext'] = maintext.strip()
            except: 
                maintext = "" 
                for i in soup.find('div', {'class': 'entry clearfix'}).find_all('div', {'dir': 'auto'}):
                    if i:
                        maintext += i.text
                article['maintext'] = maintext.strip()
