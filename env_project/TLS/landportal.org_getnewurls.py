...
direct_URLs = set()
base_url = 'https://landportal.org'

for p in range(0, 8):
    url = 'https://landportal.org/countries/771/news?page=' + str(p) 
    print("Getting urls from: ", url)
  
    soup = BeautifulSoup(scraper.get(url).text)
    for i in soup.find_all('div', {'class' : 'views-field views-field-nothing'}): 
        a_tag = i.find('a')
...
for url in final_result:
    if url:
        ...
        try:
            ...
            # Extract date using fallback only
            text = soup.get_text(separator=' ', strip=True)
            results = search_dates(text, languages=['pt'])

            if results:
                article['date_publish'] = results[0][1]
                print("Date found:", article['date_publish'])
            else:
