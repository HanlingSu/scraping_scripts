...
for p in range(120, 140): 
    link = 'https://www.mediacongo.net/articles-actualite-6_science_env-page-' + str(p) + '.html'
    ...
    for i in soup.find_all('div', {'class':'article_other_item'}):
        a_tag = i.find('a')
        if a_tag:
            href = a_tag.get('href')
            if href: 
...
for url in final_result:
    if url:
        ...
        try:
            ...
            # Extract maintext 
            try: 
                maintext = soup.find('div', {'class': 'article_ttext'}).text
                article['maintext'] = maintext.strip()
            except:
                maintext = ""
                for i in soup.find('div', {'class': 'article_ttext'}).find_all('p'):
                    if i:
                        maintext += i.text
                article['maintext'] = maintext.strip()
