...
direct_URLs = set()
categories = ['eco-et-business/agro-industrie', 'eco-et-business/energie', 'eco-et-business/leco-en', 'eco-et-business/ressources-minieres',
              'eco-et-business/industrie', 'societe/biodiversite', 'societe/environnement', 'societe/transports', ]


for c in categories: 
    url = 'https://www.journalduniger.com/category/' + str(c) + '/'
    print("Getting urls from: ", url)

    soup = BeautifulSoup(scraper.get(url).text)
    for i in soup.find_all('div', {'class':'title-left title-style04 underline04'}):
...
for url in final_result:
    if url:
        ...
        try:
            ...
            # Extract date
            try: 
                date = soup.find('div', {'class': 'post-date'}).text.strip()
                article['date_published'] = dateparser.parse(date)
            except:
