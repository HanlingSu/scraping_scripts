...
source = 'climate.gov.ph'
direct_URLs = set()


for p in range(1, 43):
    link = 'https://climate.gov.ph/newsroom/press-releases?page=' + str(p)
...
for url in final_result:
    if url:
        ...
        try:
            ...
            try: 
                date = soup.find('p', {'class': 'mb-0 mt-2'})
                # date = soup.find('h5', {'style': 'margin-bottom: 10px'})
                if date:
                    date_text = date.find('small').text.strip()
            ...
            # Extract title
            try: 
                title = soup.find('h5', {'style': 'margin-bottom: 10px'}).text
