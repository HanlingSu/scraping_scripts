...
for p in range(1, 14):
    link = 'https://www.altainews.kz/ru/pavodki2024/page/' + str(p) + '/'
    ...
    items_container = soup.find('div', {'id': 'dle-content', 'class': 'items-container'})
    
    if items_container:
        # Find all article links within the container
        for article_link in items_container.find_all('a', {'class': 'items-block'}):
            article_url = article_link.get('href')
            
            if article_url:
                # Ensure full URL
                if not article_url.startswith('http'):
                    article_url = 'https://altainews.kz' + article_url
