...
direct_URLs = set()
categories = ['environment', 'agriculture', 'oil-gas']

for c in categories: 
    for p in range(2, 56):
        link = 'https://nannews.ng/category/' + str(c) +  '/page/' + str(p) 
        print("Getting urls from: ", link)
        response = requests.get(link, verify=False)

        soup = BeautifulSoup(response.content)
        
        for i in soup.find_all('h4',{'class': 'title'}):
            try:
...
for url in final_result:
    if url:
        ...
        try:
            ...
            maintext = ""
            try:
                for i in soup.find('div', {"class" : 'content'}).find_all('p')[1:]:
                    maintext += i.text + '\n'
