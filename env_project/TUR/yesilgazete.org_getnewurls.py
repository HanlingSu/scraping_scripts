...
direct_URLs = set()
categories = ['yesil-gundem/enerji', 'yesil-gundem/ekoloji', 'yesil-gundem/iklimkrizi', 'yesil-gundem/hayvanhaklari', 'haber/yerel',
              'yesil-gundem/tarimgida', 'cop/cop29-cop', 'haber/ulasim', 'kategori/dunyadan', 'news']
pages = (185, 608, 465, 151, 144, 99, 4, 35, 568, 11)
#categories =  ['kaz-daglari', 'akbelen', 'ikizdere', 'kanal-istanbul']
#pages = (4, 10, 9, 25)

for c, max_p in zip(categories, pages):
    for p in range(1, max_p + 1):
        url = 'https://yesilgazete.org/blog/kategori/' + str(c) + '/page/' + str(p) + '/'
...
for url in final_result:
    if url:
        ...
        try:
            ...
            # Extract text 
            try: 
                maintext = soup.find('div', {'class': 'tdb-block-inner td-fix-index'}).find_all('p').text
            ...
            # Extract date 
            try:
                date = soup.find('li', {'class': 'meta-date'}).text.strip()
                print(date)
