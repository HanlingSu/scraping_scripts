...
for p in range(1, 19): 
    url = 'https://www.primicias.ec/tag/medio-ambiente/' + str(p) + '/'
    print("Getting urls from: ", url)

    soup = BeautifulSoup(scraper.get(url).text)

    for i in soup.find_all('h2', {'class':'c-article__title'}):
        a_tag = i.find('a')
...
for url in final_result:
    if url:
        ...
        try:
            ...
            # Extract text
            try:
                maintext = soup.find('div', {'class': 'body-modules'}).text
                article['maintext'] = maintext.strip()
            except:
                maintext = ""
                for i in soup.find('div', {'class': 'body-modules'}).find_all('p'):
                    if i:
                        maintext += i.text
                article['maintext'] = maintext.strip()
