...
source = 'inabio.biodiversidad.gob.ec'
direct_URLs = set()

# 53
for p in range(2, 54): 
    link = 'http://inabio.biodiversidad.gob.ec/category/noticias/page/' + str(p) 
...
for url in final_result:
    if url:
        ...
        try:
            ...
            try: 
                date = soup.find('li', {'class':'entry-date'}).text.strip()
                print(date)
                if date.lower().startswith("fecha"):
                    date = date[5:].strip()
