...
source = 'agencepressecamertest.com'
direct_URLs = set()
base_url = 'https://agencepressecamertest.com/'

for p in range(0, 2): 
    url = 'https://agencepressecamertest.com/2021-economie/2021-agro-industrie.html?start=' + str(p) 
    print("Getting urls from: ", url)

    soup = BeautifulSoup(scraper.get(url).text)

    for i in soup.find_all('div', {'class':'col-9'}):
        a_tag = i.find('a')
...
for url in final_result:
    if url:
        ...
        try:
            ...
            # Extract date 
            try:
                p_tag = soup.select_one('p[style*="font-size:9pt"]')
                if p_tag:
                    date = p_tag.text
                    date_clean = ' '.join(date.split()[:4])
