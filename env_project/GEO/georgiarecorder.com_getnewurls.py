...
for p in range(47, 48): 
    url = 'https://georgiarecorder.com/category/energy-environment/page/' + str(p) + '/'
    print("Getting urls from: ", url)

    soup = BeautifulSoup(scraper.get(url).text)
    for i in soup.find_all('div', {'class':'archiveCard'}):
...
for url in final_result:
    if url:
        ...
        try:
            ...
            # Extract date 
            try: 
                date = soup.find('h6', {'class' : 'singleBylineContainer'}).text.split('-')[-1]
