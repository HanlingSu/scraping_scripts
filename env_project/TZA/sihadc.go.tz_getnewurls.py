...
for p in range(100, 200):
    link = 'https://sihadc.go.tz/news/' + str(p) 
    print("Getting urls from: ",link)
    response = requests.get(link, verify=False)

    soup = BeautifulSoup(response.content)
    for i in soup.find('ul', {'class':'news-lists'}).find_all('li'):
    ...
    if url:
        ...
        try:
            ...
            try:
                ...
                print(date)
                if date.lower().startswith("Posted on"):
                    date = date[5:].strip()
            ...
            # Extract maintext 
            try: 
                soup.find('div', {'class': 'right-sidebar-content div-match-height'}).find_all('p')
                maintext = ""
                for i in soup.find('div', {'class': 'right-sidebar-content div-match-height'}).find_all('p'):
                    maintext += i.text
                    article['maintext'] = maintext.strip()
            except: 
                maintext = soup.find('div', {'class': 'right-sidebar-content div-match-height'}).text
                article['maintext'] = maintext.strip()
