...
base = 'https://nuevaya.com.ni/search/el/page/'
...
for c, ps, pe in zip(category, page_start, page_end):
    for p in range(ps, pe):
        ...
        soup = BeautifulSoup(reqs.text, 'html.parser')
        for i in soup.find_all('h2', {'class' : 'entry-title td-module-title'}):
            direct_URLs.append(i.find('a')['href'])
        print('Now scraped ', len(direct_URLs), 'articles from previous pages ... ')
...
for url in final_result:
    if url:
        ...
        try:
            ...
            try:
                category = soup.find('a', {'class' : 'tdb-entry-category'}).text
            except:
                category = 'News'
            print('News category:', category)
            
            if category in ['Deportes YA', 'Entretenimiento', 'Farándula Nica', 'Nicaragua Linda', 'Farándula Mundial', 'Ciencia y Tecnología YA']:
