...
# Get all <loc> tags
loc_tags = soup.findAll('loc')

# Start processing from the last link and iterate backwards
for link in loc_tags[::-1]:
    foundurl = link.text
    # Extract the year from the URL using regex
    match = year_pattern.search(foundurl)
    if match:
        year = int(match.group(1))
        # Continue if the year is greater than 2011
        if year > 2011:
            siteurls.append(foundurl)

print("Number of sitemaps found: ", len(siteurls))

#siteurls =["https://timesofindia.indiatimes.com/staticsitemap/toi/2021-April-3.xml","https://timesofindia.indiatimes.com/staticsitemap/toi/2021-April-4.xml","https://timesofindia.indiatimes.com/staticsitemap/toi/2021-April-2.xml"]
