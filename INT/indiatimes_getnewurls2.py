...
year_0 = 2022
year_1 = 2023
yearslist = list(range(year_0, year_1))

print ("Years to be updated: ", yearslist)
for yearx in yearslist:
    # defining number of pages
    pagemax = 5
                    
    pageslist = list(range(1, pagemax+1))
    for pagex in pageslist:
        # Extract URLs from sitemaps
        url = "https://www.indiatimes.com/sitemaps/" + str(yearx) + "-" + str(pagex) + "-sitemap.xml"
    ...
    print("Total number of USABLE urls found: ", len(list_urls), " for the year ", yearx)
                ...
                if "indiatimes.com" in url:
                    ...
                    try:
                        ...
                        try:
                            ...
                            #print(article['title'][0:100])
                            #print(article['maintext'][0:100])
                            print("It was duplicated + TITLE :",article['title'][0:20],"+ TEXT :",article['maintext'][0:30])
                            print("Inserted! in ", colname, " - number of urls so far: ", url_count)
                            db['urls'].insert_one({'url': article['url']})
