...
source = 'moldova-suverana.md'



category = ['/actuality/', '/outside/', '/social/']
url_count = 0
processed_url_count = 0
len_final_result = 0
page_start = [0,0,0]
page_end = [7000, 1570, 9000]
for c, ps, pe in zip(category, page_start, page_end):
    for p in range(ps, pe+1, 20):
        direct_URLs = []
        url = 'https://moldova-suverana.md/articles' + c + str(p)
        ...
        for url in final_result:
            if url:
                ...
                try:
                    ...
                    try:
                        maintext = soup.find('div',{'class':"description"}).text
                        article['maintext'] = maintext.strip()
                    except:
                        article['maintext'] = soup.find('meta', {'name' : 'description'}).text.strip()
