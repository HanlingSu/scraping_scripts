...
categories = ['politics', 'economics', 'justice', 'society', 'world', 'analytics', 'region']
page_start = [1, 1, 1, 1, 1, 1, 1]
page_end = [4, 2, 2, 5, 1, 1, 1]
url_count = 0
...
for c, ps, pe in zip(categories, page_start, page_end):
    for p in range(ps, pe+1):
        direct_URLs = []
        url = 'https://eng.ghn.ge/category/' +c +'?page=' +str(p)
        print(url)
        ...
        for url in final_result:
            if url:
                ...
                try:
                    ...
                    try:
                        date = soup.find('ul', {'class' : 'd-flex'}).text.strip().split('\n')[-1]
                    ...
                    try:
                        ...
                        article['maintext'] = soup.find('div', {'class' : 'article'}).text.strip()
