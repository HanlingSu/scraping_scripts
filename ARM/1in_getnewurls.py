...
This script updates '1in.am' using sitemaps. -> This source is protected by cloudflare
...
for j in range(initialsitemap,finalsitemap+1):
    ...
    url = 'https://www.1in.am/wp-sitemap-posts-post-' + str(j) + '.xml'
            ...
            if url == None:
                ...
                if '1in.am' in url:
                    ...
                    try:
                        ...
                        ## Fixing Date: 
                        months_arm = ["Յունուար","Փետրուար","Մարտ","Ապրիլ","Մայիս","Յունիս","Յուլիս","Օգոստոս","Սեպտեմբեր","Հոկտեմբեր","Նոյեմբեր","Դեկտեմբեր"]
                        months_arm2 = ["Հունվար","Փետրվար","Մարտ","Ապրիլ","Մայիս","Հունիս","Հուլիս","Օգոստոս","Սեպտեմբեր","Հոկտեմբեր","Նոյեմբեր","Դեկտեմբեր"]

                        contains_date = soup.find("div", {"class":"post_item_date"}).text
                        print(contains_date)
                        ...
                        # day and year:
                        daystr = vectordate[0]
                        yearstr = vectordate[2]
                        # month:
                        monthstr = vectordate[1]

                        if monthstr in months_arm:
                            monthx = months_arm.index(monthstr) + 1 
                        else:
                            monthx = months_arm2.index(monthstr) + 1 

                        article_date = datetime(int(yearstr),int(monthx),int(daystr))
