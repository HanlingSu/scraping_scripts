...
#         date = dateparser.parse(date, date_formats=['%Y-%m-%dT%H:%M']).replace(tzinfo = None) 
...
#     try:
#         #### change url pattern
#         page_url = f'https://sambadenglish.com/category/{keyword}/page/{page}/'
#         page_req = requests.get(page_url,headers=headers)
#         page_soup = BeautifulSoup(page_req.content, features="lxml")
#         for i in page_soup.find_all('a', {'class':"post-url post-title"}):
#             link = 'https://sambadenglish.com' + i['href']
...
#         keywords = [("latest-news", 6000), ("news-from-around-the-state",1000), ("politics",500), ("crime",1000), ("national-international-news",3500),
#                      ("economy",1000)]
#        #https://sambadenglish.com/category/latest-news/ #https://sambadenglish.com/category/politics/ #https://sambadenglish.com/category/crime/ 
#         # https://sambadenglish.com/category/entertainment/ 
#         #https://sambadenglish.com/category/national-international-news/india/ https://sambadenglish.com/category/economy/ #https://sambadenglish.com/category/sports/
#         #https://sambadenglish.com/category/art-culture/ #https://sambadenglish.com/category/news-from-around-the-state/
...
def sambadenglish_story(soup):
    ...
    # Get Date:
    try:
        date_contains = soup.find("time", {"class":"post-published updated"}).find('b').text
...
sections = ['/category/latest-news/', '/category/news-from-around-the-state/', '/category/politics/', '/category/crime/', '/category/crime/', 'category/national-international-news/india/', '/category/economy/']
positions = [5715, 840, 360,  720, 1860, 820]
...
for i in range(len(sections)):
  ...
  #print(section_page)
    for page in range(1, page_num +1):
        ...
        section_page = 'https://sambadenglish.com' + section_name + 'page/' + str(i) + '/'
        print(section_page)
        ...
        urls = []
        try:
            for link in soup.find('div', {'class':'col-sm-8 content-column'}).find_all('a'):
                print(link.get('href'))
                #print(link)
                urls.append(link.get('href'))
                urls = list(set(urls))
                ...
                if url == None:
                    ...
                    if "sambadenglish.com" in url:
                        ...
                        try:
                            ...
                            article['title'] = sambadenglish_story(soup)['title']
                            article['date_publish'] = sambadenglish_story(soup)['date_publish']
                            article['maintext'] = sambadenglish_story(soup)['maintext']
