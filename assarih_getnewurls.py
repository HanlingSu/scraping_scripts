...
This script updates assarih.com using daily sitemaps.
...
def assarihcom_story(soup):
    """
    Function to pull the information we want from assarih.com stories
    ...
    # Get Date
    try:
        contains_date = soup.find("abbr", {"class":"date published"})
        contains_date = str(contains_date)
        indexa = contains_date.index('title="')
        indexz = contains_date.index('">')
        datetext = contains_date[indexa+7:indexz]
        article_date = dateparser.parse(datetext)
...
url = "https://www.assarih.com/sitemap_index.xml"
...
for link in soup.findAll('loc'):
    ...
    if "post-sitemap23" in sitemap:
        siteurls.append(sitemap)
    else:
        if "post-sitemap24" in sitemap:
            siteurls.append(sitemap)
        else:
            if "post-sitemap25" in sitemap:
                siteurls.append(sitemap)
            else:
                if "post-sitemap26" in sitemap:
                    siteurls.append(sitemap)
                else:
                    pass
        ...
        if url == None:
            ...
            if "assarih.com" in url:
                ...
                try:
                    ...
                    article['title'] = assarihcom_story(soup)['title']
                    article['maintext'] = assarihcom_story(soup)['maintext']
                    article['date_publish'] = assarihcom_story(soup)['date_publish']
