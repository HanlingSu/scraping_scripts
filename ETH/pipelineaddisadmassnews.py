...
def custom_parser(soup):
    ...
    try:
        maintext = ''
        for i in soup.find_all('div',{'class':"itemFullText"}):
            for j in i.find_all('p'):
                maintext += j.text
                
        
        if len(maintext.strip()) <5:
            maintext = ''
            for i in soup.find_all('div',{'style':"text-align: start;"}):
                maintext += i.text
    ...
    # Date Parser
    try:
        for i in soup.find_all('span',{'class':"itemDateCreated"}):
            date = i.text
...
def collect_urls_sitemap(num):  
    ...
    url = f"https://ethiopianmonitor.com/wp-sitemap-posts-post-{num}.xml"
...
def collect_urls_keyword(keyword, page):
    ...
    try:
        #### change url pattern
        page_url = f'https://www.addisadmassnews.com/index.php?option=com_k2&view=itemlist&layout=category&task=category&id=1&Itemid=180&limitstart={page}'
        page_req = requests.get(page_url,headers=headers)
        page_soup = BeautifulSoup(page_req.content, features="lxml")
        for soup1 in page_soup.find_all('h3', {'class':'catItemTitle'}):
            for soup2 in soup1.find_all('a'):
                if len(soup2['href']) >3:
                    link = 'https://www.addisadmassnews.com/' + soup2['href']
                    urls.append(link)
        urls = list(set(urls))
        ...
        try:
            ...
            if _custom_parser:
                try:
                    ...
                    title, maintext, date = custom_parser(soup=soup)
...
def pipeline(sitemap,
    ...
    if keyword:
        ...
        keywords = [('empty search',60)]
        for keyword_tuple in keywords:
            
            keyword = keyword_tuple[0]
            num_page = int(keyword_tuple[1])
            
            for p in range(0,num_page):
                #collect urls
                page = p*10
                print(f'keyword: {keyword} -------------------------- {p}/{num_page} ')
