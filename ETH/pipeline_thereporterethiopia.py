...
def collect_urls_sitemap(num):  
    ...
    '''
    urls=[]
    url = f"https://thenewsnigeria.com.ng/sitemap-{num}.xml"
...
def collect_urls_keyword(keyword, page):
    ...
    try:
        #### change url pattern
        page_url = f'https://www.thereporterethiopia.com/page/{page}/?s'
        scraper = cloudscraper.create_scraper() 
        page_req = scraper.get(page_url)
        page_soup = BeautifulSoup(page_req.content, features="lxml")
        for i in page_soup.find_all('a', {'rel':'bookmark'}):
            link = i['href']
            urls.append(link)
        urls = list(set(urls))
    ...
    for j, url in enumerate(urls):
        ...
        while retries > 0:
            try:
                ...
                if response:  # If response is not None or empty
                    # Process the response as before
                    article = news_please(response=response, download_via=download_via, url=url)
                    article['source_domain'] = 'thereporterethiopia.com'
                    ...
                    if _custom_parser:
                        try:
                            ...
                            print('Error when custom parsing:', e)
                    ...
                    break  # Exit the retry loop as processing is successful

            except Exception as err:
                print(f"Attempt to scrape URL failed: {err}")
                retries -= 1  # Reduce the retry count

        if retries == 0:
            print(f'Failed to retrieve content from {url} after 3 attempts.')
    ...
    if keyword:
        ...
        for keyword_tuple in keywords:
            ...
            for page in range(0, num_page):
                retries = 3  # Number of retries if urls3 is empty
                while retries > 0:
                    ...
                    if len(urls3) > 0:
                        ...
                        # Exit the while loop once processing is done
                        break
                    else:
                        # Reduce the retry count and print a message
                        retries -= 1
                        print(f"No URLs found. Retrying... {retries} attempts left.")

                # This part executes if retries are exhausted or processing is complete
                if retries == 0:
                    print(f"Retried 3 times for page {page} but no URLs found. Continuing to next page.")
