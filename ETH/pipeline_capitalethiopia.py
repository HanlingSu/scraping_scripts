...
def custom_parser(soup):
    ...
    return title.strip(),maintext.strip(), date 
...
def collect_urls_keyword(keyword, page):
    ...
    try:
        #### change url pattern
        page_url = f'https://www.capitalethiopia.com/page/{page}/?s'
        page_req = requests.get(page_url,headers=headers)
        page_soup = BeautifulSoup(page_req.content, features="lxml")
        for i in page_soup.find_all('h3',{'class':'entry-title td-module-title'}):
            for j in i.find_all('a', {'rel':"bookmark"}):
                link = j['href']
                urls.append(link)
        urls = list(set(urls))
