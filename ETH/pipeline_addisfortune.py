...
def custom_parser(soup):
    ...
    # Title Parser: 
    try:
        title = soup.find('meta', {"name" : "og:title"})['content'].strip()
            #print('title:   ',title)
    except:
        try:
            ...
            try:
                title = soup.find('h1', {'class': 'post_title_on_singel t-center'}).text.strip()
            except:
                try:
                    title = soup.find_all('meta',property='og:title')[0]['content'].strip()
                    #print('title:   ',title)
                except:
                    title = None
    ...
    # Maintext Parser
    try:
        soup.find_all('p', {'class': 'adf_post_body'})
        maintext = ''
        for i in soup.find_all('p', {'class': 'adf_post_body'}):
            maintext += i.text

    except: 
        try: 
            soup.find('div', {'class': 'col-lg-8'}).find_all('p')
            maintext = ''
            for i in soup.find('div', {'class': 'col-lg-8'}).find_all('p'):
                maintext += i.text
            ...
            try:
                ...
                try:
                    maintext = ''
                    maintext = soup.find('p', {'class' : 'post_list_home_excerpt'}).text
                    #print('maintext:   ',maintext[:50])
    ...
    # Date Parser
    try:
        for i in soup.find( 'p', {'class':"font-11 adf_theme t-center"}):
            date = str(i.text)
        date = dateparser.parse(date).replace(tzinfo = None)
    except:
        try:
            for i in soup.find( 'p', {'class':"font-11 t-center"}):
                date = str(i.text)
            date = dateparser.parse(date).replace(tzinfo = None) 

        except:
            try:
                for i in soup.find( 'p', {'class':"font-11"}):
                    date = str(i.text)
                date = dateparser.parse(date).replace(tzinfo = None) 
            except:
                try:
                    for i in soup.find( 'p', {'class':"font-11 adf_theme"}):
                        date = str(i.text)
                    date = dateparser.parse(date).replace(tzinfo = None) 
                except:
                    try:
                        for i in soup.find( 'p', {'class':"font-11 t-center adf_theme"}):
                            date = str(i.text)
                        date = dateparser.parse(date).replace(tzinfo = None) 
                        ...
                        try:
                            date = soup.find('p', {'class' : 'font-11 adf_theme t-center'}).text.split('By')[0]
                            date = dateparser.parse(date).replace(tzinfo = None) 
                            
                        except:
                            try:
                                date  = soup.find('p', {'class' : 'font-11 t-center'}).text.split('By')[0]
                                date = dateparser.parse(date).replace(tzinfo = None) 
                                
                            except:
                                try:
                                    date = soup.find('p', {'class' : 'font-11'}).text
                                    date = dateparser.parse(date).replace(tzinfo = None) 
                                    
                                except:
                                    try:
                                        date = soup.find('p', {'class' : 'font-11 adf_theme'}).text
                                        date = dateparser.parse(date).replace(tzinfo = None) 
                                        
                                    except:
                                        try:
                                            r = soup.find('p', {'class' : 'published_on adf_theme'}).text.split('By')[0]
                                            r = re.findall('\n(.*?)\n', str(r))[1]
                                            date = r[:-7].strip()
                                            date = dateparser.parse(date).replace(tzinfo = None) 
...
def collect_urls_sitemap(num):  
    ...
    '''
    urls=[]
    # url = f"https://addisfortune.news/wp-sitemap-posts-post-{num}.xml"
    url = f'https://addisfortune.news/wp-sitemap-posts-post-{num}.xml'
    # if num == 0:
    #     # url = f'https://addisfortune.news/post-sitemap.xml'
    #     url = f'https://addisfortune.news/wp-sitemap-posts-post-1.xml'
    # else:
    #     url = f'https://addisfortune.news/post-sitemap{num}.xml'
...
def collect_urls_keyword(keyword, page):
    ...
    try:
        ...
        page_url = f'https://addisfortune.news/{keyword}/page/{page}/'
        page_req = requests.get(page_url,headers=headers)
        page_soup = BeautifulSoup(page_req.content, features="lxml")

        section =  page_soup.find('div', {"class":"container xxs-pt-mobile"})
        for i in section.find_all('a',{'class':'read_more_on_title'}):
            link = i['href']
        ...
        print(f'ERROR !!! collecting urls ----------- {page}:{err}')
        pass   
