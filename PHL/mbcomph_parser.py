...
class UpdateDB:
    ...
    def __init__(self, col_to_scrape = None,
        ...
        self.dates = pd.date_range(start=datetime(start_year,11,1), end=datetime(2023,3,31), freq='m')
    ...
    # Custom Parser:
    def mbcomph_story(self, soup):
        """
        Function to pull the information we want from mb.com.ph stories
        ...
        try:
            ...
            maintext = ''
            for sentence in content.find_all('p'):
                start = str(sentence)
                if start[3] == '<':
                    continue
                else:
                    maintext += start.lstrip('<p>').rstrip('</p>')
        ...
        try:
            ...
            date_publish = dateparser.parse(contains_date['content']).replace(tzinfo = None)
            hold_dict['date_publish'] = date_publish  
        print(date_publish)
    ...
    def update_db(self, l, yr, mo):
        ...
        for url,id in tqdm(lines):
            # FIX URL
            indexembed = url.find("/embed/")
            if indexembed > 10:
                url = url[:indexembed+1]
            else:
                url = url
            ...
            # USING cloudscraper
            # soup = BeautifulSoup(scraper.get(url, cookies=cookies).content, "html.parser")
            #soup = BeautifulSoup(scraper.get(url).content, "html.parser")
            # Fix maintext, if necessary.
            try:
                d['maintext'] = self.mbcomph_story(soup)['maintext'] #change
            except:
                d['maintext'] = None
            ...
            try:
                d['title'] = self.mbcomph_story(soup)['title'] #change
            except:
                d['title'] = None
            ...
            try:
                d['date_publish'] = self.mbcomph_story(soup)['date_publish'] #change
            except:
                d['date_publish'] = None
    ...
    def find_articles(self, url, year, month,
        ...
        query = {
                    ...
                    #'url': {'$not': re.compile('/tags', re.IGNORECASE)},
                    #'download_via': {'$in': [None, '']},
                    'title':{'$regex':"The Nation's Leading Newspaper"},
                    # 'url': {'$regex': 'mb.com.ph'},
        ...
        print('{} articles found.'.format(len(d)))
        return d
