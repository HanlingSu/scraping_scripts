        ...
        for script in soup.find_all('script', type="application/ld+json"):
            try:
                ...
                if "datePublished" in json_data:
                    ...
                    date = dateparser.parse(published_date).replace(tzinfo = None) 
                    date_found = True
                    break
            except (ValueError, TypeError):
                continue
        if not date_found:
            raise ValueError("Date not found in the provided HTML content")
        


    except:
        try:
            date_div = soup.find('div', {'class': 'a-content'})
...
def collect_urls_sitemap(num):  
    ...
    '''
    urls=[]
    url = f"https://mediaindonesia.com/sitemap_post.xml"
...
def collect_urls_year_month_day(yy, mm, days): #(keyword[0],keyword[1])
    ...
    for dd in days:
        ...
        try:
            ...
            for i in page_soup.find_all('a', {'class':"eltdf-pt-link", 'itemprop':'url'}):
                link = i['href']
                urls.append(link)
...
def collect_urls_keyword(keyword, page):
    ...
    try:
        #### change url pattern
        page_url = f'https://mediaindonesia.com/{keyword}/{page}'
        page_req = requests.get(page_url,headers=headers)
        page_soup = BeautifulSoup(page_req.content, features="lxml")
        # for i in page_soup.find_all('a', {'class':'hover-effect'}):
        #     link = i['href']
        #     urls.append(link)
        soup = page_soup.find('ul',{'class':'list-3'})
        ...
        try:
            ...
            if _custom_parser:
                try:
                    ...
                    if _date and date_publish != None:
...
def pipeline(sitemap,
    ...
    if year_month_day:
        ...
        months = ["%.2d" % i for i in range(2, 3)]
    ...
    if keyword:
        ...
        #keywords = [('',988)]
        #keywords = [('politik-dan-hukum',150),('ekonomi',150),('humaniora',200),('megapolitan',80),('nusantara',210),('internasional',60)]
        keywords = [('politik-dan-hukum',250),('ekonomi',200),('humaniora',240),('megapolitan',100),('nusantara',250),('internasional',90)]
        # keywords = [('politik-dan-hukum',10),('ekonomi',10),('humaniora',10),('megapolitan',10),('nusantara',10),('internasional',10)]
