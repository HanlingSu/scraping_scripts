...
def custom_parser(soup):
    ...
    try:
        ...
        try:
            ...
            try:
                title = soup.find('h1', {'class': 'post_title_on_singel t-center'}).text
            except:
                try:
    ...
    # Maintext Parser
    try:
        content_div = soup.find('div', {'class': 'card-isi'})
        maintext = ''
        maintext = content_div.text.strip()
        maintext = re.sub(r'\b\w+ \(\s*harianSIB\.com\s*\)', '', maintext).strip()
        maintext = re.sub(r'Baca Juga:.*?(?=\n\n)', '', maintext, flags=re.DOTALL).strip()
        maintext = re.sub(r'Editor:.*?\n', '', maintext).strip()
        maintext = re.sub(r'Editor:\s*[^\n]*', '', maintext).strip()
        maintext = re.sub(r'Editor:\s*[^E]*', '', maintext, flags=re.DOTALL).strip()
        maintext = re.sub(r'SHARE:.*?(?=\n\n)', '', maintext, flags=re.DOTALL).strip()
        maintext = re.sub(r'komentar.*$', '', maintext, flags=re.DOTALL).strip()
        maintext = re.sub(r'\n+', '\n', maintext).strip()
    except:

        try: 
            maintext = ''
            for div in soup.find_all('div', class_='leftbig'):
                paragraphs = div.find_all('div')
                for paragraph in paragraphs:
                    maintext += paragraph.get_text().strip()
            maintext = re.sub(r'Editor:\s*[^E]*', '', maintext, flags=re.DOTALL).strip()
            ...
            try:
                maintext = ''
                for div in soup.find_all('div', class_='card-isi'):
                    paragraphs = div.find_all('p')
                    for paragraph in paragraphs:
                        maintext += paragraph.get_text()
                maintext = re.sub(r'Editor:\s*[^E]*', '', maintext, flags=re.DOTALL).strip()
            except:
    ...
    # Date Parser
    try:
        for i in soup.find_all( 'time',{'class':"card-time"}):
            date = i['datetime']
...
def collect_urls_sitemap(num):  
    ...
    '''
    urls=[]
    url = f"https://www.hariansib.com/sitemap/sitemap{num}.xml"
...
def collect_urls_year_month_day(yy, mm, days): #(keyword[0],keyword[1])
    ...
    for dd in days:
        ...
        try:
            ...
            for i in soup.find_all('a', {'class':"materials__title"}):
...
def collect_urls_keyword(keyword, page):
    ...
    while retries < max_retries:
        try:
            ...
            # Change url pattern
            page_url = f'https://www.hariansib.com/kategori/{keyword}/{page}'
            page_req = requests.get(page_url, headers=headers)
            page_soup = BeautifulSoup(page_req.content, features="lxml")
            
            urls = []
            for j in page_soup.find_all('h2', {'class':"card-title"}):
                i = j.find('a')
                link = i['href']
                urls.append(link)
            
            if urls:
                urls = urls[:11]  # Keep the first 11 URLs only
                print(f'total urls:{len(urls)}')
                break  # Exit the loop if URLs are found
            else:
                print(f'No URLs found, retrying... ({retries+1}/{max_retries})')
                retries += 1
                time.sleep(retry_delay)

        except Exception as err:
            print(f'Error collecting URLs on page {page}: {err}')
            retries += 1
            time.sleep(retry_delay)
    
    if retries == max_retries and not urls:
        print(f'Max retries reached. Failed to collect URLs.')
...
def pipeline(sitemap,
    ...
    if sitemap:
        ...
        while start_date <= end_date:
            year_month_list.append((start_date.year, start_date.strftime('%m')))
            start_date += timedelta(days=32)
            start_date = start_date.replace(day=1)

        for num, date_tuple in enumerate(year_month_list):
            begin_num = 0
            end_num = len(year_month_list)
            pattern = f'{date_tuple[1]}{date_tuple[0]}'
            
            print('sitemap: ',int(num)-int(begin_num),'/', int(end_num)-int(begin_num))
            #collect urls
            urls1 = collect_urls_sitemap(pattern)
            batch_size = 20
    ...
    if year_month_day:
        ...
        for yy in years:
            for mm in months:
                mm = datetime(1, int(mm), 1).strftime("%B").lower()
                #collect urls
    ...
    if keyword:
        ...
        #keywords = [('',988)]
        #[('Headlines',110),('Marsipature-Hutanabe',30), ('Hukum',10)]
        keywords = [('Headlines',120),('Marsipature-Hutanabe',170), ('Hukum',20)]
        # keywords = [('Headlines',10),('Marsipature-Hutanabe',10), ('Hukum',5)]
        # keywords = [('Marsipature-Hutanabe',24)]
