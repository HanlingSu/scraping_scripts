...
def collect_urls_sitemap(num):  
    ...
    '''
    urls=[]
    url = f"https://www.faceofmalawi.com/wp-sitemap-posts-post-{num}.xml"
...
def collect_urls_keyword(keyword, page):
    ...
    try:
        #### change url pattern
        page_url = f'https://www.asiaterra.info/{keyword}?start={page}'
        page_req = requests.get(page_url,headers=headers)
        page_soup = BeautifulSoup(page_req.content, features="lxml")
        for i in page_soup.find_all('h3', {"class":"catItemTitle"}):
            link = "https://www.asiaterra.info" + i.find('a')['href']
            urls.append(link)
        urls = list(set(urls))
    ...
    if keyword:
        ...
        for keyword_tuple in keywords:
            ...
            for page in range(0,num_page):
                ...
                page2 = page*20
                urls3 = collect_urls_keyword(keyword=keyword, page=page2)
