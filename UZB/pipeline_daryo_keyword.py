...
def collect_urls_keyword(keyword, page):
    ...
    try:
        #### change url pattern
        page_url = f'https://daryo.uz/category/{keyword}?page={page}'
        page_req = requests.get(page_url,headers=headers)
        page_soup = BeautifulSoup(page_req.content, features="lxml")
        # for i in page_soup.find_all('a', {'class':"post_postWideCard__title__2i3ms"}):
        for i in page_soup.find_all('h2',{'class':'is-title post-title'}):
            ent = i.find('a')
            url = 'https://daryo.uz' + ent['href']
            urls.append(url)
        urls = list(set(urls))
...
def pipeline(sitemap,
    ...
    if keyword:
        ...
        # keywords = [('mahalliy',300)]
        keywords = [('dunyo',200),('markaziy-osiyo',50)]
