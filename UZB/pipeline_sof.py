...
def custom_parser(soup):
    ...
    try:
        maintext = ''
        tt = soup.find('div', {"class":"paragraph_whole"})
    ...
    # Date Parser
    try:
        for i in soup.find_all('div', {"class":"date_post_whole"}):
            date = i.find('div',{"class":'date_text'}).text.strip()
        date = dateparser.parse(date, languages = ['uz']).replace(tzinfo = None) 
...
def update_db(list_article, list_year, list_month):
    ...
    for i in range(len(list_article)):
        ...
        try:
            if "To provide the best experiences, we and our partners use technologies like cookies to store" in list_article[i]["maintext"]:
                print('trash content, continue')
                continue
            try:
...
def collect_urls_sitemap(num):  
    ...
    # dic = {}
    urls = []
    url = f"http://www.sof.uz/sitemap-index.xml"
    ...
    # all_tags = soup.find_all(['loc','lastmod'])
    # for i,tag in enumerate(all_tags):
    #     try:
    #         if tag.name == 'loc' and all_tags[i+1].name == 'lastmod':
    #             date = all_tags[i+1].text
    #             dic[tag.text] = date
    #         else:
    #             pass
    #     except:
...
def collect_urls_keyword(keyword, page):
    ...
    try:
        #### change url pattern
        page_url = f'https://sof.uz/{keyword}?page={page}'
        page_req = requests.get(page_url,headers=headers)
        page_soup = BeautifulSoup(page_req.content, features="lxml")
        content = page_soup.find('div',{'class':'news_block'})
        for i in content.find_all('a'):
            link = i['href']
            if 'sof.uz/post' in link:
                urls.append(link)
        urls = list(set(urls))
...
if __name__ == "__main__": 
    ...
    pipeline(sitemap = False, 
            ...
            keyword = True,
            _custom_parser = True,
            _title=False, _maintext=True, _date=True) # date is collected from the sitemap
