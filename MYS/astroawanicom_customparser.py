...
class UpdateDB:
    ...
    def astroawanicom_story(self, soup):
        """
        Function to pull the information we want from astroawani.com stories
        ...
        # Get Title: 
        try:
            contains_title =  soup.get_text()
            indexword = contains_title.index('|')
            article_title = contains_title[:indexword-1]
        ...
        try:
            ...
            indexword = contains_title.index('|')
            article_title = contains_title[:indexword-1]
            contains_maintext = soup.get_text()
            indexword = contains_maintext.index('HIDUPBISNESENGLISHPODCASTFOTORANCANGANTEKNOLOGI')
            maintext = contains_maintext[indexword+47+len(article_title):]
    ...
    def update_db(self, l, yr, mo):
        ...
        for url,id in tqdm(lines):
            ...
            try:
                d['maintext'] = self.astroawanicom_story(soup)['maintext'] #change
                print('The manually scraped maintext is: ', d['maintext'][0:100])
            except:
                d['maintext'] = None
                print('The manually scraped maintext is: None. ')
            ...
            try:
                d['title'] = self.astroawanicom_story(soup)['title'] #change
            except:
                d['title'] = None
            ...
            try:
                d['date_publish'] = self.astroawanicom_story(soup)['date_publish'] #change
            except:
                d['date_publish'] = None
