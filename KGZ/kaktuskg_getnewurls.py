...
len_final_result = 0
base = 'https://kaktus.kg/?lable=8&date='
for year in range(2025, 2026):
    ...
    for month in range(1, 5):   
        ...
        for day in range(1, 32):
            ...
            url = base + year_str + '-' + month_str +'-' +day_str + '&order=time'
            print(url)
        ...
        for url in final_result:
            if url:
                ...
                try:
                    ...
                    # category
                    try:
                        category = soup.find('div', {'class' : 'Article--category'}).text.strip()
                    except:
                        category = 'News'
                    
                    if category in ['Спорт', 'Пайдалуу']:
                        ...
                        try:
                            maintext = ''
                            soup.find('div', {'class' : 'Article--text'}).find_all('p')
                            for i in soup.find('div', {'class' : 'Article--text'}).find_all('p'):
                                maintext += i.text
                            article['maintext'] = maintext.strip()
                        except:
                            maintext = soup.find('div', {'class' : 'Article--text'}).text
                            article['maintext'] = maintext.strip()
