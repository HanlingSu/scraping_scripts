...
def collect_urls_sitemap(num):  
    ...
    '''
    urls=[]
    # url = f"https://www.lesahel.org/post-sitemap{num}.xml"
    url = f"https://www.lesahel.org/wp-sitemap-posts-post-{num}.xml"
...
def collect_urls_keyword(keyword, page):
    ...
    try:
        #### change url pattern
        page_url = f'https://guardian.ng/?s=&page={page}'
        page_req = requests.get(page_url,headers=headers)
        page_soup = BeautifulSoup(page_req.content, features="lxml")
        for i in page_soup.find_all('a',{"data-field":"title-link"}):
            link = i['href']
    ...
    for j, url in enumerate(urls):
        ...
        while True:
            try:
                ...
                if 'Spread the love\nSharing is caring!' in article['maintext']:
                    article['maintext'] = article['maintext'][77:]

                if 'Partager vers les réseaux' in article['maintext']:
                    article['maintext'] = article['maintext'][26:]
