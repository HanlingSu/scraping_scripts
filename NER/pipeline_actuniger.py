...
def collect_urls_keyword(keyword, page):
    ...
    try:
        #### change url pattern
        page_url = f'https://actuniger.com/{keyword}.html?start={page}'
        page_req = requests.get(page_url,headers=headers)
        page_soup = BeautifulSoup(page_req.content, features="lxml")

        # for i in page_soup.find_all('meta',{'property':'og:url'}):
        #     link = i['content']
        #     print('link collected: ', link)
        #     urls.append(link)
        for i in page_soup.find_all('h2', {'class':"article-title"}):
            s = i.find('a',{'itemprop':'url'})
            
            
            link = 'https://actuniger.com/' + s['href']
            print('link collected: ', link)
            urls.append(link)
            # print(i)
...
def pipeline(sitemap,
    ...
    if keyword:
        ...
        #keywords = [('%2520',12000)]
        keywords = [('societe',20),('international', 10),('economie',2),('politique',10),('sante-education',4) ] #330, 426, 167, 4
        for keyword_tuple in keywords:
            ...
            for page in range(0,num_page):
                ...
                urls3 = collect_urls_keyword(keyword=keyword, page=page1)
