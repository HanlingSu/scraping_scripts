...
def custom_parser(soup):
    ...
    # Date Parser
    try:
        raw_date = soup.find('time', class_='entry-date published updated').text
        date = dateparser.parse(raw_date).replace(tzinfo = None) 
...
def collect_urls_sitemap(num):  
    ...
    '''
    urls=[]
    #url = f"https://nigerinter.com/sitemap-{num}.xml"
    url = f"https://nigerexpress.info/wp-sitemap-posts-post-{num}.xml"
    # if num == 1:
    #     # url = f"http://nigerexpress.info/post-sitemap.xml"
    #     url = f"https://nigerexpress.info/wp-sitemap-posts-post-{num}.xml"
    # else:
    #     url = f"http://nigerexpress.info/post-sitemap{num}.xml"
    ...
    # Collect URLs with lastmod date after the cutoff date
    urls = []
    for url in soup.find_all('url'):
        loc = url.find('loc').text
        lastmod = url.find('lastmod').text
        lastmod_date = datetime.fromisoformat(lastmod[:-6])  # Remove timezone info
...
def collect_urls_keyword(keyword, page):
    ...
    try:
        #### change url pattern
        page_url = f'https://nigerinter.com/page/{page}/?s'
        page_req = requests.get(page_url,headers=headers)
        page_soup = BeautifulSoup(page_req.content, features="lxml")
        for soup3 in page_soup.find_all('div',{'class':'contents-wrap clearfix wrap side-spacing sb--right'}):
            for soup4 in soup3.find_all('h2', {'class':"title cb-post-title"}):
                for j in soup4.find_all('a'):
                    link = j['href']
