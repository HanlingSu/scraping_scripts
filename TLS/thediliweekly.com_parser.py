...
class UpdateDB:
    ...
    def __init__(self, col_to_scrape = None,
        ...
        # self.dates = pd.date_range(start=datetime(start_year,3,1), end=datetime.today(), freq='d')
        self.dates = pd.date_range(start=datetime(start_year,4,1), end=datetime(start_year,5,1), freq='m')
...
#################
    ...
    def kompas_story(self, soup):
        ...
        try:
            ...
            try:
                article_title = soup.find('title').text.replace('Online Khabar', '')
                hold_dict['title']  = article_title.strip()
            except:
                article_title = soup.find('h2', {'class' :'mb-0'}).text
                hold_dict['title']  =  article_title.strip()
        ...
        try:
            ...
            date = soup.find('dd', {'class' : 'rt-date-published'}).text.split(':')[-1]
            hold_dict['date_publish'] = dateparser.parse(date)
        print(hold_dict['date_publish'] )

        # Get Main Text:
        try:
            soup.find('div', {'class' : 'detail-body text-lg'}).find_all('p')
            maintext = ''
            for i in soup.find('div', {'class' : 'detail-body text-lg'}).find_all('p'):
                maintext += i.strip()
                hold_dict['maintext'] = maintext
           
        except:
            maintext = soup.find('div', {'class' : 'detail-body text-lg'}).text
            hold_dict['maintext'] = maintext
    ...
    def find_articles(self, url, year, month,
        ...
        query = {
            ...
            #         { '$or': [ { 'maintext': { '$in': ['', None] }, 'maintext' : {"$regex" : 'someting'} } } ] }]
                    ...
                    # 'url': {'$regex': 'https://www.diariolibre.com/actualidad/internacional/.*'},
        }            
