...
sitemap_base = 'https://newsroompanama.com/wp-sitemap-posts-post-'
...
for url in final_result[::-1]:
    if url:
        ...
        try:
            ...
            category = ["News"]
            for i in soup.find('div', {'class'  : 'entry-header-details'}).find_all('li', {'class' : 'meta-category'}):
                category.append(i.text.strip())
            print(category)

            blacklist = ['TRAVEL', 'ENTERTAINMENT', 'ADVERTISING']
            if any(c in blacklist for c in category):
