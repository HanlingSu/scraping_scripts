...
page_start = 1
page_end = 15
base = 'https://telegraf.al/category/opinion/page/'
...
for p in range(page_start, page_end+1):
    ...
    # print(soup)
    item = soup.find('div', {'class' : 'td-pb-span8 td-main-content'}).find_all('h3', {'class' : 'entry-title td-module-title'})
    for i in item:
    ...
    if url:
        ...
        try:
            ...
            try:
                ...
                maintext = ''
                for i in soup.find('div', {'class' : 'td-post-content td-pb-padding-side'}).find_all('p',  {'style' : None, 'class' : None}):
                    maintext += i.text.strip()
                article['maintext']  = maintext
            except:          
                try:
                    maintext = soup.find('div', {'class' : 'td-post-content'}).text.strip()
                    article['maintext']  = maintext
                except:
