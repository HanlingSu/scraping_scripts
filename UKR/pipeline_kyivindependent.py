...
def clean_article_text(maintext):
    """
    Clean the maintext of an article by removing introductory text.
    
    Parameters:
    maintext (str): The original text of the article.
    
    Returns:
    str: The cleaned text of the article.
    """
    # Define patterns for introductory text
    patterns = [
        r"Support independent journalism in Ukraine. Join us in this fight\.\s*\n+\s*Become a member\s*\n+\s*Support us just once\s*\n+"
    ]

    for pattern in patterns:
        match = re.search(pattern, maintext)
        if match:
            return maintext[match.end():].strip()
...
def custom_parser(soup):
    ...
    try:
            maintext = ''
            maintext += soup.find('div',{'class':'c-content'}).text.strip()
            # if "Editor’s Note:" in maintext or "Editor’s note:" in maintext or "Editor's Note:" in maintext or "Editor's note:" in maintext:
            maintext = re.sub("Editor’s Note:", '', maintext).strip()
            maintext = re.sub("Editor’s note:", '', maintext).strip()
            maintext = re.sub("Editor's Note:", '', maintext).strip()
            maintext = re.sub("Editor's note:", '', maintext).strip()
            maintext = re.sub("Key developments on [A-Z][a-z]+ \d{1,2}:", "", maintext).strip()
    ...
    for i in range(len(list_article)):
        try:
            ...
            if l ==[]:
                ...
                print(f'{i+1}/{len(list_article)}: update success!: {colname} ---- (title){list_article[i]["title"][:40]}****(main){list_article[i]["maintext"][:40]}')
...
def collect_urls_keyword(keyword, page):
    ...
    try:
        #### change url pattern
        # page_url = f'https://kyivindependent.com/tag/{keyword}/page/{page}'
        page_url = f'https://kyivindependent.com/author/{keyword}/page/{page}'
        page_req = requests.get(page_url,headers=headers)
        page_soup = BeautifulSoup(page_req.content, features="lxml")
        for i in page_soup.find_all('a',{'rel':'dofollow','class':'c-card__link'}):
...
def get_content(urls, _custom_parser, download_via, _title, _maintext, _date):
    ...
    for j, url in enumerate(urls):
        ...
        try:
            ...
            article['maintext'] = clean_article_text(article['maintext'])
...
if __name__ == "__main__": 
    ...
    pipeline(sitemap = False, # no longer useful
