...
def collect_urls_keyword(keyword, page):
    ...
    try:
        #### change url pattern
        page_url = f'https://delo.ua/ru/search/?date_from={keyword}-01-01&date_to={keyword}-12-31&page={page}&rubrics%5B0%5D=politics&rubrics%5B1%5D=economy&rubrics%5B2%5D=finance&rubrics%5B3%5D=society&rubrics%5B4%5D=opinions'
        page_req = requests.get(page_url,headers=headers)
...
def pipeline(sitemap,
    ...
    if sitemap:
        ...
        for num in selected_months:
            ...
            try:
                ...
                begin_num = selected_months[0]
                end_num = selected_months[-1]
                print('sitemap: ',int(num)-int(begin_num),'/', int(end_num)-int(begin_num))
    ...
    if keyword:
        ...
        keywords = [i for i in range(2015,2023)]
