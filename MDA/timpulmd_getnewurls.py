    ...
    for url in final_result:
        if url:
            ...
            try:
                ...
                try:
                    category = soup.find('header', {'class' : 'entry-header-outer'}).text.strip().split('/')[1]
                except:
                    category = 'News'
                print(category)
                if category in ['Opinii și Editoriale', 'Istorie', 'Timpul Liber']:
                ...
                article['maintext'] = article['maintext'].replace(article['title'], '').strip()
