...
def custom_parser(soup):
    ...
    try:
        ...
        # article_date = re.findall('"datePublished": "(.*)",', txt)[0]
        date = dateparser.parse(article_date).replace(tzinfo = None) 

    except:
        try:
            article_date = json.loads(soup.find('script', type = 'application/ld+json').contents[0], strict=False)[0]['datePublished']
            date = dateparser.parse(article_date).replace(tzinfo = None) 
        except:
            try:
                s = soup.find('script',type="application/ld+json").string
                article_date = re.findall('"datePublished": "(.*)","dateModified"', s)[0]
                date = dateparser.parse(article_date).replace(tzinfo = None)
    ...
    for i in range(len(list_article)):
        try:
            ...
            if l ==[]:
                ...
                print(f'{i+1}/{len(list_article)}: updating!: {colname} ---- (title){list_article[i]["title"][:40]}****(main){list_article[i]["maintext"][:40]}')
...
def collect_urls_year_month_day(yy, mm, days): #(keyword[0],keyword[1])
    ...
    for dd in days:
        ...
        try:
            #### change url pattern
            page_url = f'https://media-cdn.t24.com.tr/media/sitemaps/sitemap-{yy}{mm}{dd}.xml'
...
def pipeline(sitemap,
    ...
    if sitemap:
        ...
        '''        
        list_total_url = []
        total_url = f'https://www.sozcu.com.tr/tools/sitemaps/x/sitemapindex_all.php'
        ...
        ####
        all_months_urls = []
        list_total_url = list_total_url[1:5]
        
        
        
        #####
        for url in list_total_url:
            urls1 = collect_urls_sitemap(num=url)
            for url2 in urls1:
                all_months_urls.append(url2)

        
        for i, url in enumerate(all_months_urls):
            # if i <1800:
            #     continue
            ...
            if len(all_months_urls) > batch_size:
                batched_lists = split_list(all_months_urls, batch_size = batch_size)
    ...
    if year_month_day:
        ...
        months_1 = ["%.2d" % i for i in range(11,13)]
            ...
            for mm in months:
                ...
                if len(urls2) > batch_size:
                    ...
                    for index, ll in enumerate(batched_lists):
                        ...
                        #get content and fix it 
                        print(f'{yy}-{mm}: ({index}/{len(batched_lists)})')
...
if __name__ == "__main__": 
    ...
    pipeline(sitemap = False,
            ...
            keyword = False,
            _custom_parser = True,
            _title=False, _maintext=True, _date=True)
