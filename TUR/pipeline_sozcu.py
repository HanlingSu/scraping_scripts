...
def collect_urls_sitemap(num):  
    ...
    urls=[]
    # url = num
    url = 'https://www.sozcu.com.tr/feeds/sitemapwpposts'
...
def collect_urls_year_month_day(yy, mm, days): #(keyword[0],keyword[1])
    ...
    try:
        #### change url pattern
        page_url = f'https://www.sozcu.com.tr/feeds/sitemapposts?year={yy}&month={mm}'
...
def pipeline(sitemap,
    ...
    if sitemap:
        ...
        ###
        all_months_urls = []
        list_total_url = list_total_url[2:]
        print(list_total_url)
        ...
        all_months_urls = collect_urls_sitemap(num=1)
        # all_months_urls = pd.read_csv('sozcu_new_links.csv')['url']
